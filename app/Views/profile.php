					...
					<form id="profileForm" method="post">
						<?php if (!empty($user)): ?>
							...
							<input type="tel" name="phone" id="phone" class="form-control"
								value="<?= esc($user['cust_Phone']) ?>" pattern="^\d{7,15}$" maxlength="15" required
							...
							<div class="card p-2 mb-2">
								...
								<div class="mt-2">
									...
									<?php if (!empty($addr['add_Default']) && $addr['add_Default'] == 1): ?>
										| <span>Default</span>
									<?php else: ?>
										| <a href="javascript:void(0);" onclick="setDefaultAddress(<?= $addr['add_Id'] ?>)">Set
											as Default</a>
									<?php endif; ?>
								</div>
							</div>
	...
	<div class="modal-dialog">
		<form id="editAddressForm" class="modal-content">
			...
			<div class="modal-body">
				...
				<div>&nbsp</div>
				<input type="tel" maxlength="15" minlength="7" pattern="^\d{7,15}$" name="add_Phone" id="add_Phone"
					class="form-control" placeholder="Phone" required
					oninvalid="this.setCustomValidity('Phone number must be between 7 to 17 digits.')"
					oninput="this.setCustomValidity('')">
